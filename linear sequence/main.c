...
{
    if (!value) {
	fprintf(stderr, "test_assert_impl\n");
	test_fail();
	}
}
...
void test_assert_seq_impl(LSQ_HandleT seq, int count, ...)
{
	...
	fprintf(stderr, "LSQ_GetSize(seq) != count\n");	
	test_fail();
}
    va_start(vl, count);
    ...
    for (it = LSQ_GetFrontElement(seq); !LSQ_IsIteratorPastRear(it); LSQ_AdvanceOneElement(it))
    {
		...
		fprintf(stderr, "(*LSQ_DereferenceIterator(it) != va_arg(vl, int)) \n");
        fprintf(stderr, "     %i %i \n", *LSQ_DereferenceIterator(it), va_arg(vl, int));
		test_fail();
	}
        count--;
    }
